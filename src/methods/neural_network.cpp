...
torch::Tensor solution_to_tensor(const Solution &solution) {
    torch::Tensor one_hot_encoding =
        torch::zeros({parameters->nb_colors, graph->nb_vertices});
    for (int vertex = 0; vertex < graph->nb_vertices; ++vertex) {
        if (solution[vertex] != -1) {
            one_hot_encoding.index_put_({solution[vertex], vertex}, 1);
        }
    }
...
NeuralNetwork::NeuralNetwork(int64_t nb_colors,
    ...
    int64_t size_layer1 = graph->nb_vertices;
    int64_t size_layer2 = graph->nb_vertices + nb_operators / 2;
...
std::vector<int> NeuralNetwork::predict(torch::Tensor input) {
    ...
    auto size_output = tensor_predictions.sizes()[1];

    std::vector<int> predictions(size_output);

    for (int i = 0; i < size_output; ++i) {
        predictions[i] = tensor_predictions[0][i].item<int>();
    }
